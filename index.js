...
let finals2014 = fifaData.filter(function (x) {
...
function getYears(array, getFinalsCB) {
  return (array = getFinalsCB(array).map((x) => x.Year));
}
...
function getWinners(array, getFinalsCB) {
  return getFinalsCB(array).map((x) =>
 ...
 */
function getWinnersByYear(data, getFinalsCB, getWinnersCB, getYearsCB) {
...
function getCountryWins(data, teamInitials) {
  const totWins = data.reduce(function (acc, currentMatch) {
    if (currentMatch["Home Team Initials"] === teamInitials) {
      if (currentMatch["Home Team Goals"] > currentMatch["Away Team Goals"]) {
        console.log(
    ...
    } else if (currentMatch["Away Team Initials"] === teamInitials) {
      if (currentMatch["Away Team Goals"] > currentMatch["Home Team Goals"]) {
        console.log(
          currentMatch["Away Team Goals"],
          currentMatch["Home Team Goals"]
        );
